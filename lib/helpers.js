...
// Busca un resumen corto en Wikipedia para usar como reseña cuando Google no tiene una propia.
async function obtenerResumenWikipedia(nombre, idioma) {
  try {
    const buscarUrl = `https://${idioma}.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(
      nombre
    )}&limit=1&namespace=0&format=json&origin=*`;
    const buscarResp = await fetch(buscarUrl);
    const buscarData = await buscarResp.json();
    const titulo = buscarData?.[1]?.[0];
    if (!titulo) return null;

    const resumenUrl = `https://${idioma}.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(titulo)}`;
    const resumenResp = await fetch(resumenUrl);
    if (!resumenResp.ok) return null;
    const resumenData = await resumenResp.json();

    if (resumenData.type === "disambiguation" || !resumenData.extract) return null;

    let texto = resumenData.extract.split(". ")[0];
...
module.exports = { distanciaKm, estimarViaje, obtenerClima, obtenerResumenWikipedia };
